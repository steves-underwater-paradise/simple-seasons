...
public class SetSeasonCommand {
	...
	public static int execute(CommandContext<ServerCommandSource> commandContext, ServerCommandSource source) {
		...
		if (seasonArgument > Seasons.WINTER.ordinal() || seasonArgument < Seasons.SPRING.ordinal()) {
			source.sendError(
					Text.literal(String.format("Season argument can only be between 0-3 (inclusive), given: %s.", seasonArgument)));
			return 1;
		}

		var currentSeason = getSeason(source.getServer());
		if (seasonArgument == currentSeason) {
			source.sendError(Text.literal(
					String.format("Season argument (%s) is the same as the current season (%s).", seasonArgument, currentSeason)));
			return 1;
		}

		setSeason(source.getServer(), seasonArgument);
		source.sendMessage(Text.literal(
				String.format("Set season to %s (%s).", seasonArgument, Seasons.getName(seasonArgument).toLowerCase(Locale.ROOT))));
